...
TAXONOMY_OTHER_REVIEW_PATH = OPP_EXPERIMENT / "taxonomy_v3" / "analysis" / "contemporary_other_review_final.csv"
...
VIEW_TITLES = {
    "Summary verification": "Sentence source alignment",
    "Source coverage": "Information coverage",
    "Evaluator agreement": "Evaluator agreement on the contemporary validation subset",
    "Study overview": "Study overview",
    "OPP source alignment": "Sentence source alignment",
    "OPP source coverage": "Information coverage",
    "Taxonomy mapping": "Taxonomy mapping across the two corpora",
}
...
def load_human_coverage_ratings() -> pd.DataFrame:
    if HUMAN_COVERAGE_RATINGS_PATH.exists():
        needed = ["comparison_id", "blind_id", "unit_id", "evaluator", "coverage_label"]
        available = pd.read_csv(HUMAN_COVERAGE_RATINGS_PATH, nrows=0).columns
        return pd.read_csv(
            HUMAN_COVERAGE_RATINGS_PATH,
...
def load_opp_data() -> tuple[pd.DataFrame, ...]:
    ...
    return (read(OPP_METADATA_PATH), read(OPP_KEY_PATH), read(OPP_CLAIMS_PATH),
            ...
            read(TAXONOMY_OTHER_REVIEW_PATH))
...
st.markdown(
    ...
    .pv-global-context { color:var(--pv-muted); font-size:13px; text-align:right; padding:8px 2px; white-space:nowrap; }
    ...
    [data-testid="stSegmentedControl"] button[aria-pressed="true"] { background:#173d67 !important; color:#fff !important; border-color:#173d67 !important; }
    ...
    .pv-not-evaluated { margin-top:10px; padding:13px 14px; border:1px dashed #b8c5d2; border-radius:8px; background:#f6f8fa; color:var(--pv-text); }
    .pv-not-evaluated strong { display:block; color:var(--pv-navy); margin-bottom:4px; }
    .pv-not-evaluated span { color:var(--pv-muted); font-size:12px; line-height:1.45; }
...
# The final adjudicated coverage file is sufficient for the prototype. Loading
# the much larger raw A1-A3 ratings table on every Streamlit rerun wastes memory.
human_coverage_ratings = pd.DataFrame()
...
opp_view = view in {"OPP source alignment", "OPP source coverage", "Taxonomy mapping"}

with st.container(key="pv_topbar"):
    global_page = view in {"Study overview", "Taxonomy mapping"}
    ...
    prompt_set = st.session_state.get(prompt_key, "basic")
    if not global_page:
        ...
        with summary_col:
            st.markdown('<div class="pv-global-context">Complete 30-policy study · 540 summaries</div>', unsafe_allow_html=True)
    ...
    if not global_page:
        ...
        summary_index = summary_options[0]
...
with nav_col:
    with st.container(key="pv_nav"):
        st.markdown('<div class="pv-kicker">Study</div>', unsafe_allow_html=True)
        if st.button("○  Study overview", key="pv_nav_Study overview", type="primary" if view == "Study overview" else "secondary", use_container_width=True):
            st.session_state.pv_view = "Study overview"
            st.rerun()
        st.markdown('<div class="pv-kicker" style="margin-top:18px">Contemporary policies</div>', unsafe_allow_html=True)
        for label, display, icon in (("Summary verification", "Source alignment", "✓"), ("Source coverage", "Information coverage", "≡"), ("Evaluator agreement", "Evaluator agreement", "≋")):
            if st.button(f"{icon}  {display}", key=f"pv_nav_{label}", type="primary" if view == label else "secondary", use_container_width=True):
                st.session_state.pv_view = label
                st.rerun()
        st.markdown('<div class="pv-kicker" style="margin-top:18px">OPP-115 policies</div>', unsafe_allow_html=True)
        for label, display, icon in (("OPP source alignment", "Source alignment", "✓"), ("OPP source coverage", "Information coverage", "≡")):
            if st.button(f"{icon}  {display}", key=f"pv_nav_{label}", type="primary" if view == label else "secondary", use_container_width=True):
                st.session_state.pv_view = label
                st.rerun()
        st.markdown('<div class="pv-kicker" style="margin-top:18px">Compare corpora</div>', unsafe_allow_html=True)
        if st.button("↔  Taxonomy mapping", key="pv_nav_Taxonomy mapping", type="primary" if view == "Taxonomy mapping" else "secondary", use_container_width=True):
            st.session_state.pv_view = "Taxonomy mapping"
            st.rerun()
        if not global_page:
            st.markdown(f'<div class="pv-side-note">Policy: {html.escape(policy_display)}<br>Blind ID: {html.escape(blind_id)}</div>', unsafe_allow_html=True)

with main_col:
    context_meta = "Complete study · 30 policies · 540 summaries" if global_page else f'{html.escape(policy_display)} · {html.escape(SETS[prompt_set])} · {html.escape(MODELS[model])} · {html.escape(STRATEGIES[strategy])}'
    st.markdown(f'<div class="pv-context"><h1>{VIEW_TITLES[view]}</h1><span class="pv-meta">{context_meta}</span></div>', unsafe_allow_html=True)
    ...
    if view == "Summary verification":
    ...
    elif view in {"OPP source alignment", "OPP source coverage"}:
        inspection = "Sentence source alignment" if view == "OPP source alignment" else "Information coverage"
        ...
        if inspection == "Sentence source alignment":
            ...
            if claims.empty:
                ...
                with left:
                    with st.container(key="pv_left"):
                        st.markdown('<div class="pv-panel-head"><h2>Plain-English summary</h2><div class="pv-legend">Gemini decision&nbsp; <span><span class="pv-dot" style="background:#16794a"></span>Supported</span><span><span class="pv-dot" style="background:#9b6200"></span>Partial</span><span><span class="pv-dot" style="background:#b42318"></span>Unsupported</span></div></div>', unsafe_allow_html=True)
                        with st.container(key="pv_claim_list"):
                            for index, claim in claims.reset_index(drop=True).iterrows():
                                ...
                                with st.container(key=f"pv_claim_{state}_opp_{index}"):
                                    if st.button(str(claim["claim_text"]), key=f"pv_opp_claim_{blind_id}_{index}", type="primary" if index == chosen else "secondary", use_container_width=True):
                                        ...
                                        st.rerun()
                        st.download_button("Download summary", summary_text, file_name=f"{stem}.txt", mime="text/plain", key="pv_download_opp_summary")
                with right:
                    ...
                    with st.container(key="pv_right"):
                        st.markdown('<div class="pv-panel-head"><h2>Evidence and judgement</h2></div>', unsafe_allow_html=True)
                        st.markdown('<div class="pv-inspector"><p class="pv-section-intro">The same evaluator layout is used for both corpora. OPP-115 contains automated results only; researcher validation was reserved for the contemporary subset.</p></div>', unsafe_allow_html=True)
                        ...
                        with st.container(key="pv_eval_card_human_opp"):
                            st.markdown('<div class="pv-evaluator-head"><div><div class="pv-evaluator-name">3. Researchers</div><div class="pv-evaluator-desc">Independent researcher validation</div></div></div>', unsafe_allow_html=True)
                            st.markdown('<div class="pv-not-evaluated"><strong>Not evaluated for OPP-115</strong><span>Full researcher assessment was conducted on the three contemporary policies.</span></div>', unsafe_allow_html=True)
        else:
            ...
            if cov.empty:
                ...
                if st.session_state.get("pv_opp_unit_blind") != blind_id:
                    st.session_state.pv_opp_unit_blind, st.session_state.pv_opp_unit_index = blind_id, 0
                selected = min(int(st.session_state.get("pv_opp_unit_index", 0)), len(cov) - 1)
                item = cov.iloc[selected]
                ...
                with left:
                    with st.container(key="pv_left"):
                        st.markdown('<div class="pv-panel-head"><h2>Important source information</h2><div class="pv-legend">Gemini decision&nbsp; <span><span class="pv-dot" style="background:#16794a"></span>Covered</span><span><span class="pv-dot" style="background:#9b6200"></span>Partial</span><span><span class="pv-dot" style="background:#b42318"></span>Not covered</span></div></div>', unsafe_allow_html=True)
                        with st.container(key="pv_unit_list"):
                            for index, unit_row in cov.iterrows():
                                state, _, _, _ = verdict_style(str(unit_row.get("coverage_label", "")))
                                with st.container(key=f"pv_unit_{state}_opp_{index}"):
                                    unit_button, unit_status = st.columns([.84, .16], gap="small", vertical_alignment="center")
                                    with unit_button:
                                        if st.button(f"{unit_row['important_information']}\nSource unit {unit_row['unit_id']}", key=f"pv_opp_coverage_{blind_id}_{index}", type="primary" if index == selected else "secondary", use_container_width=True):
                                            st.session_state.pv_opp_unit_index = index
                                            st.rerun()
                                    with unit_status:
                                        st.markdown(f'<div class="pv-unit-status {state}">{html.escape(str(unit_row.get("coverage_label", "")))}</div>', unsafe_allow_html=True)
                with right:
                    with st.container(key="pv_right"):
                        st.markdown('<div class="pv-panel-head"><h2>Evidence and judgement</h2></div>', unsafe_allow_html=True)
                        st.markdown('<div class="pv-inspector"><p class="pv-section-intro">Coverage asks whether each important source item appears in the summary. The same inspection layout is used for both corpora.</p></div>', unsafe_allow_html=True)
                        with st.container(key="pv_eval_card_coverage_minicheck_opp"):
                            st.markdown('<div class="pv-evaluator-head"><div><div class="pv-evaluator-name">1. MiniCheck</div><div class="pv-evaluator-desc">Binary source-support model</div></div></div>', unsafe_allow_html=True)
                            st.markdown('<div class="pv-not-evaluated"><strong>Not applicable to coverage</strong><span>MiniCheck evaluates generated sentences against a source; it does not test whether important source information was omitted.</span></div>', unsafe_allow_html=True)
                        with st.container(key="pv_eval_card_coverage_gemini_opp"):
                            st.markdown('<div class="pv-evaluator-head"><div><div class="pv-evaluator-name">2. Gemini</div><div class="pv-evaluator-desc">Evidence-citing coverage evaluator</div></div></div>', unsafe_allow_html=True)
                            st.markdown(badge_html(str(item["coverage_label"]), str(item["unit_id"])), unsafe_allow_html=True)
                            ...
                            st.markdown(f'<div class="pv-reason-grid"><div class="pv-reason-part"><strong>Evidence in summary</strong>{html.escape(str(item.get("summary_evidence", "No corresponding evidence found.")))}</div><div class="pv-reason-part"><strong>Coverage result</strong>{html.escape(str(item.get("coverage_label", "")))}</div></div>', unsafe_allow_html=True)
                        with st.container(key="pv_eval_card_coverage_human_opp"):
                            st.markdown('<div class="pv-evaluator-head"><div><div class="pv-evaluator-name">3. Researchers</div><div class="pv-evaluator-desc">Independent researcher validation</div></div></div>', unsafe_allow_html=True)
                            st.markdown('<div class="pv-not-evaluated"><strong>Not evaluated for OPP-115</strong><span>Researcher coverage assessment was conducted on the three contemporary policies.</span></div>', unsafe_allow_html=True)
        ...
        with st.container(key="pv_single_panel"):
            ...
            st.markdown('<p class="pv-section-intro">Important information was identified before the OPP taxonomy was applied. This page therefore shows whether independently extracted historical and contemporary source units can be organised by the ten OPP-115 categories, and which units still require review.</p>', unsafe_allow_html=True)
            a, b, c = st.columns(3)
            ...
            reviewed_other = taxonomy_other_review[taxonomy_other_review["final_category"] == "Other"] if not taxonomy_other_review.empty else pd.DataFrame()
            st.markdown(
                f'<div class="pv-callout"><strong>What “Other” means:</strong> {len(reviewed_other)} contemporary items remained Other after researcher review. '
                'This flags information that does not fit neatly; it does not by itself prove that a new category is needed. '
                'A defensible extension would require a distinct privacy practice that recurs across several independent policies.</div>',
                unsafe_allow_html=True,
            )
            if not taxonomy_difference.empty:
                ...
                chart = alt.Chart(comparison).mark_bar().encode(
                    ...
                    row=alt.Row("Corpus:N", title=None), tooltip=["category", "Corpus", alt.Tooltip("Percent:Q", format=".2f")]
            ...
            st.markdown('<div class="pv-label">Inspect individual taxonomy mappings</div>', unsafe_allow_html=True)
            dataset_labels = {
                "OPP historical": "Historical OPP-115 source units",
                "Contemporary human reference": "Contemporary researcher source units",
                "Contemporary Gemini validated addition": "Validated contemporary Gemini additions",
            }
            selected_dataset = st.segmented_control(
                "Mapping dataset",
                list(dataset_labels),
                format_func=dataset_labels.get,
                default="OPP historical",
                key="pv_taxonomy_dataset",
            )
            dataset_maps = taxonomy_mapping[taxonomy_mapping["dataset"] == selected_dataset].copy()
            policy_options = sorted(dataset_maps["policy_id"].astype(str).unique())
            selected_policy = st.selectbox(
                "Policy",
                policy_options,
                index=policy_options.index(policy_id) if policy_id in policy_options else 0,
                key=f"pv_taxonomy_policy_{selected_dataset}",
            )
            category_options = ["All categories"] + sorted(dataset_maps["primary_category"].astype(str).unique())
            selected_category = st.selectbox("Category", category_options, key=f"pv_taxonomy_category_{selected_dataset}")
            policy_maps = dataset_maps[dataset_maps["policy_id"].astype(str) == selected_policy].copy()
            if selected_category != "All categories":
                policy_maps = policy_maps[policy_maps["primary_category"] == selected_category]
            policy_maps = policy_maps.reset_index(drop=True)
            if policy_maps.empty:
                st.info("No taxonomy mapping matches this policy and category filter.")
            else:
                ...
                official = taxonomy_validation[(taxonomy_validation["policy_id"] == selected_policy) & (taxonomy_validation["unit_id"] == mapped["unit_id"])] if selected_dataset == "OPP historical" else pd.DataFrame()
                ...
                review = taxonomy_other_review[
                    (taxonomy_other_review["policy_id"].astype(str) == selected_policy)
                    & (taxonomy_other_review["unit_id"].astype(str) == str(mapped["unit_id"]))
                ] if not taxonomy_other_review.empty else pd.DataFrame()
                reviewed_category = str(review.iloc[0]["final_category"]) if not review.empty else str(mapped["primary_category"])
                review_reason = str(review.iloc[0]["final_reason"]) if not review.empty else "No separate researcher review was required."
                fourth_title = "Official OPP overlap check" if selected_dataset == "OPP historical" else "Researcher review"
                fourth_body = (
                    f"{official_categories}<br>Compatible: {compatible}"
                    if selected_dataset == "OPP historical"
                    else f"Final category: {reviewed_category}<br>{review_reason}"
                )
                st.markdown(f'<div class="pv-reason-grid"><div class="pv-reason-part"><strong>Important information</strong>{html.escape(str(mapped["important_information"]))}</div><div class="pv-reason-part"><strong>Mapped category</strong>{html.escape(str(mapped["primary_category"]))}<br><span class="pv-muted">Confidence: {html.escape(str(mapped["confidence"]))}</span></div><div class="pv-reason-part"><strong>Mapping reason</strong>{html.escape(str(mapped["explanation"]))}</div><div class="pv-reason-part"><strong>{html.escape(fourth_title)}</strong>{fourth_body}</div></div>', unsafe_allow_html=True)
                st.markdown('<div class="pv-label pv-why-label">Original-policy evidence</div>', unsafe_allow_html=True)
                contemporary_ids = {"P01": "PILOT01", "P02": "PILOT02", "P03": "PILOT03"}
                source_policy = contemporary_ids.get(selected_policy, selected_policy)
                mapped_source = load_opp_source(source_policy) if selected_dataset == "OPP historical" else load_source(source_policy)
                st.markdown(source_reader_html(mapped_source, str(mapped["exact_source_evidence"]), False), unsafe_allow_html=True)
                if reviewed_category == "Other":
                    st.warning("Category decision: retain this item as Other for the present study. One item—or several items from only three policies—is insufficient evidence for a stable new category.")
            st.markdown('<div class="pv-callout"><strong>Limitation:</strong> only three contemporary privacy policies were examined. Other current policies may contain important practices that do not fit the existing OPP-115 categories and could justify refinement or additional categories in future work.</div>', unsafe_allow_html=True)
        ...
        with st.container(key="pv_single_panel"):
            ...
            st.markdown('<p class="pv-section-intro">The prototype presents the complete dissertation study and makes its evaluation process inspectable. It connects generated summaries to sentence source-alignment evidence, information coverage and separate automated and researcher decisions.</p>', unsafe_allow_html=True)
            st.markdown('<div class="pv-callout"><strong>Complete study design:</strong> 30 privacy policies × 3 model families × 3 prompting strategies × 2 prompt versions = 540 summaries. The corpus contains 27 OPP-115 policies and three contemporary consumer-technology policies.</div>', unsafe_allow_html=True)
            st.markdown('<div class="pv-study-flow"><div class="pv-flow-card"><strong>1 · Generate</strong>GPT, Llama and Mistral summarise all 30 policies under matched prompt conditions.</div><div class="pv-flow-card"><strong>2 · Measure accessibility</strong>Reading Ease, word count and relative length describe readability and conciseness.</div><div class="pv-flow-card"><strong>3 · Verify in two directions</strong>Sentence-to-source checks alignment; source-to-summary checks information coverage.</div><div class="pv-flow-card"><strong>4 · Validate evaluators</strong>MiniCheck and Gemini are compared with researcher decisions on the contemporary subset.</div></div>', unsafe_allow_html=True)
            a, b, c, d = st.columns(4)
            a.metric("Privacy policies", "30", help="27 OPP-115 and three contemporary policies")
            b.metric("Generated summaries", f"{len(summaries) + len(opp_summaries):,}")
            c.metric("Evaluated sentences", f"{len(gemini_scores) + len(opp_gemini):,}")
            d.metric("Coverage comparisons", f"{len(human_coverage) + len(opp_coverage):,}")
            if not metrics.empty:
                ...
                st.markdown('<div class="pv-label">Contemporary validation subset: accessibility and conciseness</div>', unsafe_allow_html=True)
                st.caption("Each point represents one of the 54 contemporary summaries. The complete 540-summary statistical results are reported in the dissertation.")
